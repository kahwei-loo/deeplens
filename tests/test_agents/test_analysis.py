...
@patch("deeplens.agents.analysis.sentiment_analyzer")
@patch("deeplens.agents.analysis.compute_statistics")
def test_analysis_mixed_skips_web_analysis(mock_stats, mock_sentiment):
    """Articles + comments → runs sentiment, skips web analysis (comments take precedence)."""
    ...
    assert result["sentiment"]["total_analyzed"] == 2
    # Web analysis is skipped when comments exist
    assert "web_analysis" not in result
