...
def supervisor_agent(state: DeepLensState) -> dict:
    ...
    try:
        decision: SupervisorDecision = structured_llm.invoke(  # type: ignore[assignment]
            [
        ...
        # Fallback: always force report if we have data OR are at/near iteration limit.
        # Without this guard, a repeated LLM failure could drive iteration_count past
        # max_iter indefinitely by always returning "research".
        at_limit = (iteration + 1) >= max_iter
        fallback = "report" if (has_analysis or has_web_data or at_limit) else "research"
        return {
            "next_agent": fallback,
