...
- Mixed path: statistics, sentiment, AND web analysis — all three run together
"""
...
def _sanitize_web_content(text: str) -> str:
    """Strip control characters from web content before embedding in LLM prompts.

    Removes null bytes and non-printable control characters that could disrupt
    LLM tokenisation or be used for prompt injection. Normal whitespace (\\t, \\n,
    \\r) is preserved intentionally.
    """
...
def _analyze_web_articles(articles: list[WebArticle]) -> WebAnalysis | None:
    ...
    # Build article context for the LLM — sanitize content to remove control
    # characters before embedding third-party web text into the prompt.
    article_texts: list[str] = []
    limit = get_settings().article_llm_chars
    for a in articles:
        title = _sanitize_web_content(a.get("title", "Untitled"))
        domain = a.get("source_domain", "unknown")
        content = _sanitize_web_content(a.get("content", ""))
    ...
    # with_structured_output guarantees a WebAnalysisResult instance on success.
    # If parsing fails, LangChain raises an exception caught by the caller.
    response: WebAnalysisResult = structured_llm.invoke(  # type: ignore[assignment]
        [
            {
                "role": "system",
                "content": (
                    ...
                    "The articles are from third-party web sources."
                ),
            },
            {
                "role": "user",
...
def analysis_agent(state: DeepLensState) -> dict:
    ...
    - Web articles available: run LLM-based web analysis (themes, entities, summary)
    - Both: run all three — statistics, sentiment, and web analysis
    """
    ...
    # Run web article analysis whenever we have articles — including mixed path
    # (web-only OR alongside YouTube data; both produce valuable insights)
    if web_articles:
        try:
