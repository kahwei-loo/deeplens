...
def research_agent(state: DeepLensState) -> dict:
    ...
    if plan_instructions:
        # Join all accumulated instructions so the LLM sees the full history
    ...
    try:
        plan: ResearchPlan = structured_llm.invoke(  # type: ignore[assignment]
            [
    ...
    if plan.youtube_enrichment and settings.youtube_available:
        ...
        try:
            ...
            # Use the LLM-refined yt_query (entity name) instead of the raw user
            # query so that "Research Baby Monster" → searches "Baby Monster".
            if (
                plan.entity_type in ("artist/group", "brand")
                and not existing_channel
            ):
                channel = youtube_channel(channel_name=yt_query)
                if channel:
    ...
    # Deduplicate web results by URL across iterations, keeping highest score
    _seen_urls: dict[str, WebResult] = {}
    for r in existing_web + new_web_results:
        url = r["url"]
