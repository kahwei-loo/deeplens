...
def _tavily_search(query: str, max_results: int, search_depth: str) -> dict[str, Any]:
    ...
    return cast(
        dict[str, Any],
        client.search(query=query, max_results=max_results, search_depth=search_depth),
    )
...
def web_search(
    ...
    try:
        ...
        settings = get_settings()
        safe_msg = (
            str(exc).replace(settings.tavily_api_key, "***")
            if settings.tavily_api_key
            else str(exc)
        )
        logger.error("web_search error: %s", safe_msg)
        return []
...
def extract_urls(urls: list[str]) -> list[WebArticle]:
    ...
    try:
        ...
        for item in response.get("results", []):
            ...
            limit = get_settings().article_content_chars
            content = raw[:limit] if raw else ""
            articles.append(
                WebArticle(
                    url=url,
